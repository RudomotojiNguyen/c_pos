...
class _OtpFormWidgetState extends State<OtpFormWidget> {
    ...
    return BlocListener<CustomerBloc, CustomerState>(
      ...
      listener: (context, state) {
        ...
        if (state is CheckOTPSuccess) {
          widget.onCompleted(
              state.point, state.amount, getOtp, customerInfoOtp);
  ...
  Widget _countdown(int data) {
    return Text.rich(
      TextSpan(text: 'Gửi lại mã trong ', children: [
        TextSpan(
          text: data.formatCountdownTimer(),
        )
      ]),
    );
  }
              ...
              children: [
                if (!getIt.get<Configurations>().isProduct) ...[
                  TextSpan(
                    ...
                    style: AppFont.t
                        .s()
                        .copyWith(color: AppColors.informationColor),
  ...
  void onSubmit() {
    if (customerInfoOtp?.id != null || widget.customerId != null) {
      _customerBloc.add(CheckOTPEvent(
        type: widget.type,
